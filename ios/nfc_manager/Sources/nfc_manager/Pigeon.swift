...
class HostApiPigeonSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: HostApiPigeon?, messageChannelSuffix: String = "") {
    ...
    let miFareSendMiFareCommandMultipleChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.nfc_manager.HostApiPigeon.miFareSendMiFareCommandMultiple\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      miFareSendMiFareCommandMultipleChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let handleArg = args[0] as! String
        let commandPacketsArg = args[1] as! [FlutterStandardTypedData]
        api.miFareSendMiFareCommandMultiple(handle: handleArg, commandPackets: commandPacketsArg) { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
      ...
      miFareSendMiFareCommandMultipleChannel.setMessageHandler(nil)
    }
