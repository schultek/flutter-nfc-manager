...
interface HostApiPigeon {
  ...
  fun nfcATransceiveMultiple(handle: String, bytesList: List<ByteArray>): List<ByteArray>
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: HostApiPigeon?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.nfc_manager.HostApiPigeon.nfcATransceiveMultiple$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val handleArg = args[0] as String
            val bytesListArg = args[1] as List<ByteArray>
            val wrapped: List<Any?> = try {
              listOf(api.nfcATransceiveMultiple(handleArg, bytesListArg))
            } catch (exception: Throwable) {
              PigeonPigeonUtils.wrapError(exception)
            }
            reply.reply(wrapped)
